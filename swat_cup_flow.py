...
if __name__ == '__main__':
    ...
    parser.add_argument('-o', '--output',    default='SwatCup', metavar='output_file', help='Output directory for plots (default: %(default)s).')
    parser.add_argument('-b', '--subbasin',  default=[1], type=int,  nargs='*', help='Desired subbasin index/indices to save (default: %(default)s).')
    parser.add_argument('-u', '--usgs', nargs='*', help='The USGS site numbers for each reach')
    parser.add_argument('-l', '--log', action='store_true',  default=False, help='Prefix for plot file names')
    parser.add_argument('-w', '--weight', nargs='*', type=float,   default=[1.0], help='weight of the streamflow for each site in the objective function')
